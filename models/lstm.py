...
class LSTM(nn.Module):
	...
	def init_hidden(self):
		...
		for i in range(self.layers):
			h.append(Variable(torch.zeros(self.batch_size, self.hidden_size)))
			c.append(Variable(torch.zeros(self.batch_size, self.hidden_size)))
...
if __name__ == '__main__':
	torch.set_device(0)
	net = LSTM(128+5,256,5,2)
	inp = Variable(torch.FloatTensor(3,128+5))
	out = net(inp)
	print ('out: ', out.shape)